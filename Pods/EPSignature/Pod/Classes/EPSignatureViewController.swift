...
@objc public protocol EPSignatureDelegate {
    ...
    @objc optional    func epSignature(_: EPSignatureViewController, isSubscriptionRequired : Bool)
}
...
open class EPSignatureViewController: UIViewController {
    ...
    override open func viewDidLoad() {
        ...
        btnSubscription.setImage(UIImage(named:"check.png"), for: .normal)
    }
    
    @IBAction func actionSubscribe(_ sender: UIButton) {
        
        if (btnSubscription.currentImage?.isEqual(UIImage(named: "uncheck.png")))!
        {
            btnSubscription.setImage(UIImage(named:"check.png"), for: .normal)
            signatureDelegate?.epSignature!(self, isSubscriptionRequired: true)
        }
        else
        {
            btnSubscription.setImage(UIImage(named:"uncheck.png"), for: .normal)
            signatureDelegate?.epSignature!(self, isSubscriptionRequired: false)
        }
    }
